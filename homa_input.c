...
void homa_message_in_init(struct homa_message_in *msgin, int length, int unscheduled) {
                           ...
                           struct homa_sock *hsk, struct homa_server_rpc *srpc)
{
    ...
    if (srpc->request.bytes_remaining == 0) {
        ...
        printk(KERN_NOTICE "[homa_data_from_client] Incoming request is complete\n");
        srpc->state = SRPC_READY;
        list_add_tail(&srpc->ready_links, &hsk->ready_server_rpcs);
        sk->sk_data_ready(sk);
    }else{
        printk(KERN_NOTICE "[homa_data_from_client] Incoming response till have remaining:%d\n",srpc->request.bytes_remaining);
    }
}
//usage:[handler]
...
void homa_data_from_server(struct homa *homa, struct sk_buff *skb, struct homa_sock *hsk, struct homa_client_rpc *crpc)
{
    ...
    if (crpc->state != CRPC_INCOMING) {
        ...
        homa_message_in_init(&crpc->response, ntohl(h->message_length), ntohl(h->unscheduled));
    ...
    if (crpc->response.bytes_remaining == 0) {
        ...
        printk(KERN_NOTICE "[homa_data_from_server] Incoming response till have remaining:%d\n",crpc->response.bytes_remaining);
    }
}
