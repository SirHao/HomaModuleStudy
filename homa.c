...
long sysctl_homa_mem[3]
__read_mostly;  //系统内存用的
...
homa_init(void) {
    ...
    if (status != 0) {
        printk(KERN_ERR
        "proto_register failed in homa_init: %d\n",
                status);
        goto out;
    }
    //注册top-level proto
    ...
    if (status != 0) {
        printk(KERN_ERR
        "inet_add_protocol failed in homa_init: %d\n",
                status);
        goto out_unregister;
    }
...
*option) {
printk(KERN_WARNING
"unimplemented getsockopt invoked on Homa socket:"
" level %d, optname %d\n", level, optname);
return -
EINVAL;
...
int homa_handler(struct sk_buff *skb) {
    ...
    "[homa_handler] network header size %lu, memory allocated %lu\n", skb->data -
                                                                      skb_network_header(skb), atomic_long_read(
