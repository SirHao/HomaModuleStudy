...
//[proto_ops]此结构定义处理Homa套接字上各种操作的函数。 这些函数是相对通用的：top level systemcall。
//其中许多操作都可以通过独立于Homa协议的PF_INET功能实现。
...
//[proto]此结构还定义了处理Homa套接字上各种操作的函数。
//但是，这些函数比homa_proto_ops中的函数低级：它们特定于PF_INET协议家族，并且在许多情况下，它们由homa_proto_ops中的函数调用。
//这些功能大多数都有特定于Homa的实现。
...
//[net_protocol]IP使用此结构将传入的Homa数据包传递给我们。
...
/**
 * homa_load(): kernel model init func
 * @return: 0 on success, otherwise a negative errno.
 */
static int __init homa_load(void) {
	int status;
	printk(KERN_NOTICE "[init]Homa module loading\n");
	...
	if (status != 0) {
		printk(KERN_ERR "[homa init]inet_add_protocol failed in homa_init: %d\n",
		    status);
		goto out_unregister;
	}
...
/**
 * homa_unload():unloaded from the Linux kernel.
 */
...
/**
 * homa_bind() - 并未执行实际系统调用 ，只是绑定端口 (因为是一个类似udp的数据报协议
...
void homa_close(struct sock *sk, long timeout) {
	...
	homa_sock_destroy(hsk,&homa.port_map);			//free hsk 's variable
...
int homa_ioc_recv(struct sock *sk, unsigned long arg) {
	...
	if (unlikely(copy_from_user(&args, (void *) arg, offsetof(struct homa_args_recv_ipv4, source_addr))))
		return -EFAULT;
	err = import_single_range(READ, args.buf, args.len, &iov, &iter);  //迭代器，用来更方便获取userspace参数
	if (unlikely(err))
		return err;

	lock_sock(sk);
	while (1) {
		if (!list_empty(&hsk->ready_server_rpcs)) {		//如果handler收到了这个hsk完整的server_rpc
			struct homa_server_rpc *srpc;
			srpc = list_first_entry(&hsk->ready_server_rpcs, struct homa_server_rpc, ready_links); //获取到第一个准备好的srpc（如果后续还是这个数据结构，那么可以改进为showtest pkg in x）
			list_del(&srpc->ready_links);
			srpc->state = SRPC_IN_SERVICE;             //改变state
			msgin = &srpc->request;
			source = &srpc->client;
			args.id = srpc->id;
			break;
		}
		if (!list_empty(&hsk->ready_client_rpcs)) {		//如果handler收到了这个hsk完整的client_rpc的response
		...
		//如果都没有并且是阻塞的
		timeo = sock_rcvtimeo(sk, noblock);   			// 当socket为阻塞时，获取timeout的值
		timeo = homa_wait_ready_msg(sk, &timeo);		// 获取剩余的timeo
	...
	release_sock(sk);
	if (unlikely(copy_to_user( &((struct homa_args_recv_ipv4 *) arg)->source_addr, &args.source_addr, sizeof(args) - offsetof(struct homa_args_recv_ipv4, source_addr))))
		return -EFAULT;
	return result;
...
/**
 * homa_ioc_reply() - The top-level function for the ioctl that implements the homa_reply user-level API
 * 和ioc_send区别在于，从一个bind的server端口，发送回原来的client而不是从一个全新的client port发送
 * @sk:       Socket for this request.
 * @arg:      Used to pass information from/to user space.
 *
 * Return: 0 on success, otherwise a negative errno.
...
int homa_ioc_reply(struct sock *sk, unsigned long arg) {
	...
	if (unlikely(copy_from_user(&args, (void *) arg, sizeof(args))))
		return -EFAULT;
	err = import_single_range(WRITE, args.response, args.resplen, &iov, &iter);
	if (unlikely(err))
		return err;
...
int homa_ioc_send(struct sock *sk, unsigned long arg) {
	...
	if (unlikely(copy_from_user(&args, (void *) arg, sizeof(args))))  return -EFAULT;
...
/**
 * homa_wait_ready_msg() - 等待直到出现一个ready的rpc
...
{
	DEFINE_WAIT_FUNC(wait, woken_wake_function);  //定义一个func:woken_wake_function,并赋值给 @wait

	int rc;

	add_wait_queue(sk_sleep(sk), &wait); //该方法的功能是将等待队列项@wait 挂到等待队列头sk_sleep(sk)中。
	...
	remove_wait_queue(sk_sleep(sk), &wait);//该方法的功能是将等待队列项@wait从等待队列头sk_sleep(sk)中移除。
	return rc;
}
 ...
 * Return: ??
 */
__poll_t homa_poll(struct file *file, struct socket *sock, struct poll_table_struct *wait) {
