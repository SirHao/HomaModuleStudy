...
void homa_sock_init(struct homa_sock *hsk, struct homa *homa)
{
	...
	hlist_add_head_rcu(&hsk->client_links.hash_links, &socktab->buckets[homa_port_hash(hsk->client_port)]);
...
void homa_sock_destroy(struct homa_sock *hsk, struct homa_socktab *socktab)
{
	...
	if (hsk->server_port != 0)  hlist_del_rcu(&hsk->server_links.hash_links);
	mutex_unlock(&socktab->writeLock);
 ...
 */
int homa_sock_bind(struct homa_socktab *socktab, struct homa_sock *hsk, __u16 port)
{
	int result = 0;
	...
	if (owner != NULL) {
		if (owner != hsk) result = -EADDRINUSE;
		goto done;
	}
	if (hsk->server_port) {
	...
	hlist_add_head_rcu(&hsk->server_links.hash_links, &socktab->buckets[homa_port_hash(port)]);
    done:
	mutex_unlock(&socktab->writeLock);
	return result;
}
...
struct homa_sock *homa_sock_find(struct homa_socktab *socktab,  __u16 port)
{
	...
	hlist_for_each_entry_rcu(link, &socktab->buckets[homa_port_hash(port)], hash_links) {
